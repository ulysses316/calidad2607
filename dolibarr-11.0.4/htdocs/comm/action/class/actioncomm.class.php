...
class ActionComm extends CommonObject
{
	...
	{$this->userassigned = array($userownerid=>array('id'=>$userownerid, 'transparency'=>$this->transparency));}
        ...
        if ($resql)
        {
			...
			if (!$error) && (!empty($this->socpeopleassigned))
				{
    ...
    public function fetch_userassigned($override = true)
    {
        ...
        if ($resql2)
        {
            ...
            while ($obj = $this->db->fetch_object($resql2))
            {
                ...
                if ($override === true) && (empty($this->userownerid))
                    {
    ...
    public function getNomUrl($withpicto = 0, $maxlength = 0, $classname = '', $option = '', $overwritepicto = 0, $notooltip = 0, $save_lastsearch_value = -1)
    {
		...
		if (empty($conf->global->AGENDA_USE_EVENT_TYPE)) && ($this->type_code != 'AC_OTH_AUTO') {$labeltype = $langs->trans('ActionAC_MANUAL');
		}
        ...
        if ($withpicto) && (!empty($conf->global->AGENDA_USE_EVENT_TYPE))	// Add code into ()
            {
            	if ($labeltype)
            	{
